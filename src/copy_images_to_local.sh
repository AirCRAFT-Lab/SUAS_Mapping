...
ssh suas@192.168.0.238 << 'EOF'
  ...
  # Creates temporary directory to contain lates images
  ...
  # Check if target directory is clean
  if [ -d /home/suas/latest_images ]; then
    echo "Previous 'latest_images' directory found. Removing..."
  ...
  echo "Copying latest 'Images' directory to Jetson..."
